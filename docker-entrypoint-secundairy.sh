...
echo "=================================================="
echo "PostgreSQL Streaming Replication Entrypoint"
...
if cluster_exists; then
    ...
    if [ -n "$PRIMARY" ]; then
        ...
        echo "ℹ No primary found - fresh initialization as standalone"
        /usr/local/bin/docker-entrypoint.sh postgres --initialize-only > /tmp/pg_init.log 2>&1 || true
    fi
fi
...
# Step 3: Configure recovery mode (standby.signal)
echo "Step 3: Configuring node role..."

if [ "$NODE_TYPE" = "replica" ]; then
    echo "   This is a REPLICA node (locked standby)"
...
EOF
    ...
    echo "✓ REPLICA mode: recovery configured (node locked)"
else
    echo "   This is a BACKUP node (can be promoted)"
    # Remove standby.signal if exists to allow promotion
    rm -f "$RECOVERY_SIGNAL"
    rm -f "$PGDATA/recovery.conf"
    
    # Create standby.signal template for potential promotion (only if replicating)
    if cluster_exists && grep -q "primary_conninfo\|standby_mode" "$POSTGRESQL_CONF" 2>/dev/null; then
        echo "✓ BACKUP mode: Configured for streaming replication"
    else
        echo "✓ BACKUP mode: Fresh initialization or primary role"
    fi
fi
